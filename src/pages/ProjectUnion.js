...
// Include these durations
const INCLUDED_DURATIONS = new Set(["7D", "30D", "3M", "6M"]);

// Rank → score mapping (capped at 1000)
const RANK_CAP = 1000;
const rankToScore = (rank) => {
  if (!isFinite(rank) || rank <= 0) return 0;
  if (rank > RANK_CAP) return 0;
  return (RANK_CAP - rank + 1) / RANK_CAP; // 1.0 .. 0.001
};
// IMPORTANT: rank fallback must be weaker than mindshare
const RANK_BACKOFF = 0.35;

export default function ProjectUnion() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        // Keep UNION + included durations (with 7D)
        ...
        if (!cancelled) {
          ...
          document.title = `UNION • ${username} • Yapium`;
        }
      } catch (e) {
        if (!cancelled) {
  ...
  // Combined score + eligibility
  const { totalScore, eligible, bestRank } = useMemo(() => {
    if (!entries.length)
      return { totalScore: 0, eligible: false, bestRank: null };
    ...
    for (const e of entries) {
      ...
      const mindshare =
        typeof e?.mindshare === "number" ? Math.max(0, e.mindshare) : null; // clamp ≥ 0
      ...
      if (mindshare !== null) {
        // Mindshare wins when present
        score += mindshare * tWeight * tierWeight;
      } else {
        // Fallback to rank with strong back-off so it can't dominate
    ...
    // Eligible if they appear on ANY UNION leaderboard we kept
    const isEligible = entries.length > 0;
    return {
      totalScore: score,
      eligible: isEligible,
  ...
  const tokensAwarded = useMemo(() => {
    if (!totalScore || GLOBAL_TOTAL_SCORE <= 0) return 0;
    const share = totalScore / GLOBAL_TOTAL_SCORE;
        ...
        <div className="lg:col-span-6 space-y-4">
          <GlassPanel highlight="orange">
            ...
            <div className="mt-5 grid grid-cols-2 gap-4 md:grid-cols-4">
              ...
              <MiniStat
                label="TOKENS"
                value={`${formatQty(tokensAwarded, 2)} $U`}
              />
              <MiniStat
                label="Token Price ($)"
...
function CardModal({ onClose, username, tokens, worth, fdv, tagline }) {
  ...
  async function downloadPNG() {
    ...
    img.onload = () => {
      ...
      a.href = pngURL;
      a.download = `yapium_${username}_union_card.png`;
      a.click();
    };
    img.src = url;
  }
