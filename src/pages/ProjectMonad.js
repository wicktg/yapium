...
/**
 * MONAD Project Dashboard
 * - topic_id === "MONAD"
 * - Time weighting: 6M > 3M > 12M > 30D
 * - Leaderboards weighting: tier1 (Creator) > tier2 (Community). No "specific".
 * - Rewards pool: 500M $MON out of 100B total supply
 * - Global mindshare pool: 390 (assumption)
...
const REWARD_POOL = 500_000_000; // 500M MON
const GLOBAL_MINDSHARE = 390; // assumed global mindshare

// Time weights: 6M > 3M > 12M > 30D
const TIME_WEIGHTS = { "6M": 1.0, "3M": 0.85, "12M": 0.7, "30D": 0.5 };
// Leaderboard weights (no "specific")
const TIER_WEIGHTS = { tier1: 1.0, tier2: 0.7 };
// Included durations
const INCLUDED_DURATIONS = new Set(["6M", "3M", "12M", "30D"]);

// Rank-fallback tuning
...
export default function ProjectMonad() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        // Keep MONAD + included durations
        const filtered = data.filter(
          (d) => d?.topic_id === "MONAD" && INCLUDED_DURATIONS.has(d?.duration)
        );

        if (!cancelled) {
          setEntries(filtered);
          setStatus({ loading: false, error: null });
          document.title = `MONAD • ${username} • Yapium`;
        }
      } catch (e) {
        if (!cancelled) {
        ...
        <div className="lg:col-span-6 space-y-4">
          <GlassPanel highlight="orange">
              ...
              <div>
                ...
                {!status.loading && (
                  <div className="mt-2 text-[11px] text-white/50">
                    Basis: {usedMindshare ? "Mindshare" : "Rank (fallback)"}
                  </div>
                )}
              </div>
              {status.loading ? (
            ...
            <div className="mt-5 grid grid-cols-2 gap-4 md:grid-cols-4">
              ...
              <MiniStat
                label="TOKENS"
                value={`${formatQty(tokensAwarded, 2)} $MON`}
              />
              <MiniStat
                label="Token Price ($)"
          ...
          <GlassPanel>
            ...
            <ul className="pl-5 text-sm text-white/70 space-y-2 list-disc">
              <li>
                ...
                <b>100,000,000,000 $MON</b>.
              </li>
              <li>
                <span className="text-white/80">Rewards Pool for Yappers:</span>{" "}
                <b>500,000,000 $MON</b>.
              </li>
              <li>
                <span className="text-white/80">Timeframe Weightage:</span>{" "}
                <b>6M</b> &gt; <b>3M</b> &gt; <b>12M</b> &gt; <b>30D</b>.
              </li>
              <li>
  ...
  async function handleCompare() {
    ...
    try {
      ...
      for (const e of filtered) {
        ...
        if (mindshare !== null) {
          sawMS = true;
          msScore += mindshare * tWeight * tierWeight;
        } else {
          rankScore += rankToUnitScore(rank) * tWeight * tierWeight;
        }
      }

      const finalScore =
        sawMS && msScore > 0 ? msScore : rankScore * RANK_PENALTY;

      const tokens =
        finalScore && GLOBAL_MINDSHARE > 0
          ? REWARD_POOL * (finalScore / GLOBAL_MINDSHARE)
          : 0;
              ...
              <div className="mt-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                <GlassPanel>
                  ...
                  <div className="text-sm text-white/70">
                    Tokens: <b>{formatQty(you.tokens, 2)} $MON</b>
                  </div>
                  <div className="text-sm text-white/70">
                ...
                <GlassPanel>
                  ...
                  <div className="text-sm text-white/70">
                    Tokens: <b>{formatQty(friend.tokens, 2)} $MON</b>
                  </div>
                  <div className="text-sm text-white/70">
...
function CardModal({ onClose, username, tokens, worth, fdv, tagline }) {
  ...
  async function downloadPNG() {
    ...
    img.onload = () => {
      ...
      a.href = pngURL;
      a.download = `yapium_${username}_monad_card.png`;
      a.click();
    };
    img.src = url;
  }
