...
// Rank fallback: keep lighter than mindshare path
const RANK_PENALTY = 0.35;
function rankToUnitScore(rank) {
  if (!Number.isFinite(rank) || rank <= 0) return 0;
  if (rank > 1000) return 0;
  return (1001 - rank) / 1000; // 1.0 for #1 → ~0 for #1000
}

export default function ProjectCysic() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        // Keep CYSIC + included durations
        const filtered = data.filter(
          (d) => d?.topic_id === "CYSIC" && INCLUDED_DURATIONS.has(d?.duration)
        );

        if (!cancelled) {
          setEntries(filtered);
          setStatus({ loading: false, error: null });
          document.title = `CYSIC • ${username} • Yapium`;
        }
      } catch (e) {
        if (!cancelled) {
        ...
        <div className="lg:col-span-6 space-y-4">
          <GlassPanel highlight="orange">
            ...
            <div className="mt-5 grid grid-cols-2 gap-4 md:grid-cols-4">
              ...
              <MiniStat
                label="TOKENS"
                value={`${formatQty(tokensAwarded, 2)} $CYS`}
              />
              <MiniStat
                label="Token Price ($)"
...
function CardModal({ onClose, username, tokens, worth, fdv, tagline }) {
  ...
  async function downloadPNG() {
    ...
    img.onload = () => {
      ...
      a.href = pngURL;
      a.download = `yapium_${username}_cysic_card.png`;
      a.click();
    };
    img.src = url;
  }
