 ...
 * - Eligibility: appear on leaderboard; mindshare preferred, rank as fallback
 */
...
export default function ProjectPortalBTC() {
  ...
  // Weighted score (mindshare preferred, rank fallback)
  const { weightedScore, usedMindshare, eligible, bestRank } = useMemo(() => {
    ...
    for (const e of entries) {
      const duration = e._duration; // normalized "3M", "30D", "6M"
      const tier = e._tier; // normalized "tier1"/"tier2"
      const ms =
    ...
    if (sawMindshare && wm > 0) {
      return {
        weightedScore: wm,
        usedMindshare: true,
        eligible: entries.length > 0, // eligible if present on any board
    ...
    return {
      weightedScore: wr,
      usedMindshare: false,
      eligible: entries.length > 0 && wr > 0, // present + nonzero fallback score
      bestRank: isFinite(minRank) ? minRank : null,
    };
  }, [entries]);
