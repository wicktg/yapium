...
/**
 * HANA Project Dashboard
 * - topic_id === "HANAHANA"
 * - Ticker: $HANA
 * - Total supply: 1B
 * - Rewards pool: 15,000,000 $HANA (1.5% of supply)
 * - Time weighting: 3M > 30D > 6M > 7D (7D included)
 ...
 *   • Prefer mindshare (if any present)
 *   • If ALL mindshare are null/absent → fallback to rank (reduced via penalty)
 * - FDV slider: $0 → $5B, price = FDV / totalSupply
 */

const TOTAL_SUPPLY = 1_000_000_000; // 1B
const REWARD_POOL = 15_000_000; // 15M HANA
const GLOBAL_MINDSHARE = 400; // assumed global mindshare pool like other projects

// Time weights (3M > 30D > 6M > 7D)
const TIME_WEIGHTS = { "3M": 1.0, "30D": 0.8, "6M": 0.6, "7D": 0.4 };
// Leaderboard weights (tier1 = Creator, tier2 = Community)
const TIER_WEIGHTS = { tier1: 1.0, tier2: 0.7 };
// Included durations
const INCLUDED_DURATIONS = new Set(["3M", "30D", "6M", "7D"]);

// Rank fallback tuning (deliberately weaker than mindshare)
const RANK_PENALTY = 0.35;
function rankToUnitScore(rank) {
  if (!Number.isFinite(rank) || rank <= 0) return 0;
  if (rank > 1000) return 0;
  return (1001 - rank) / 1000; // rank #1 ≈ 1.0, rank #1000 ≈ 0.001
}

export default function ProjectHana() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        // Keep only HANAHANA + included durations
        const filtered = data.filter(
          (d) =>
            d?.topic_id === "HANAHANA" && INCLUDED_DURATIONS.has(d?.duration)
        );

        if (!cancelled) {
          setEntries(filtered);
          setStatus({ loading: false, error: null });
          document.title = `HANA • ${username} • Yapium`;
        }
      } catch (e) {
        if (!cancelled) {
  ...
  // Weighted score (mindshare preferred, rank fallback if ALL mindshare missing)
  const { weightedScore, eligible, bestRank } = useMemo(() => {
    ...
    let wm = 0; // mindshare path
    let wr = 0; // rank fallback path (used only when mindshare absent)
        ...
        <div className="lg:col-span-6 space-y-4">
          <GlassPanel highlight="orange">
            ...
            <div className="mt-5 grid grid-cols-2 gap-4 md:grid-cols-4">
              ...
              <MiniStat
                label="TOKENS"
                value={`${formatQty(tokensAwarded, 2)} $HANA`}
              />
              <MiniStat
                label="Token Price ($)"
        ...
        <div className="px-5 pb-5">
          ...
          {(you || friend) && (
            <>
              ...
              <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <GlassPanel highlight="orange">
                  ...
                  <div className="mt-2 text-sm">
                    Tokens: <b>{formatQty(you?.tokens ?? 0, 2)} $HANA</b>
                  </div>
                ...
                <GlassPanel>
                  ...
                  <div className="mt-2 text-sm">
                    Tokens: <b>{formatQty(friend?.tokens ?? 0, 2)} $HANA</b>
                  </div>
                  <div className="text-sm">
              ...
              {/* Head-to-head bar (two colors like IRYS/MONAD) */}
              <div className="mt-6">
...
function CardModal({ onClose, username, tokens, worth, fdv, tagline }) {
  ...
  async function downloadPNG() {
    ...
    img.onload = () => {
      ...
      a.href = pngURL;
      a.download = `yapium_${username}_hana_card.png`;
      a.click();
    };
    img.src = url;
  }
