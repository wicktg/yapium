...
export default function ProjectBoundless() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        // Keep only BOUNDLESS + included durations
        const filtered = data.filter(
          (d) =>
            d?.topic_id === "BOUNDLESS" && INCLUDED_DURATIONS.has(d?.duration)
        );

        if (!cancelled) {
          setEntries(filtered);
          setStatus({ loading: false, error: null });
          document.title = `BOUNDLESS • ${username} • Yapium`;
        }
      } catch (e) {
        if (!cancelled) {
        ...
        <div className="lg:col-span-6 space-y-4">
          <GlassPanel highlight="orange">
            ...
            <div className="mt-5 grid grid-cols-2 gap-4 md:grid-cols-4">
              ...
              <MiniStat
                label="TOKENS"
                value={`${formatQty(tokensAwarded, 2)} $ZKC`}
              />
              <MiniStat
                label="Token Price ($)"
...
function CardModal({ onClose, username, tokens, worth, fdv, tagline }) {
  ...
  async function downloadPNG() {
    ...
    img.onload = () => {
      ...
      a.href = pngURL;
      a.download = `yapium_${username}_boundless_card.png`;
      a.click();
    };
    img.src = url;
  }
...
function NeonButton({ children, className = "", ...props }) {
  ...
  return (
    <button
      {...props}
      className={`relative inline-flex items-center justify-center gap-1 rounded-xl px-4 py-2 text-sm font-medium ${
        disabled
          ? "bg-[#FF7A29] text-black cursor-not-allowed"
          : "bg-[#FF7A29] text-black hover:scale-[1.02]"
      } shadow-[0_0_24px_rgba(255,122,41,0.5)] ${className}`}
